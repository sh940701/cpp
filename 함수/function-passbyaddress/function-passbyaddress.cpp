...
// 윗줄의 변수 argv는 윗윗줄에 나열되어 있는 배열 데이터가 저장되어 있는 메모리의 주소값 담고있는 변수이다.


// 위 main함수 예시에서, 포인터를 요소로 가지고 있는 배열을 받을 때 char* argv
void func(int argc, char argv[][5]) {

// 아래 선언 방법에 대해 알아보자. 사실은 위와 같다고 볼 수 있다.
// 메모리에 data가 { {"a", "b", "c", "d"}, {"e", "f", "g", "h"} } 형식으로 저장되어있다고 할 때,
// 위에서 선언한 char argv[][5]의 경우, [5]앞의 빈 배열[]표시는 data내에서 가장 큰 배열을 뜻한다.
// 가장 큰 배열 안에 char type요소 5개로 이루어진 배열들이 있다는 것이다.
...
// void func(int argc, char (*argv)[5]) {
    ...
    cout << "here is: " << (*argv) << endl;
    cout << typeid(argv).name() << endl; // PA5_c -> 요소가 char인 길이 5의 배열을 가리키는 포인터
